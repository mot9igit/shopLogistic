...
if ($_REQUEST['pageId']) {
	if ($_SESSION['sl_filters'][$_REQUEST['pageId'] . '_' . $scriptProperties['type'] . '_' . $scriptProperties['col_id']]) {
		$scriptProperties['where'] = $_SESSION['sl_filters'][$_REQUEST['pageId'] . '_' . $scriptProperties['type'] . '_' . $scriptProperties['col_id']];
	}
}
...
if (!empty($rows) && is_array($rows)) {
	...
	foreach ($rows as $k => $row) {
		...
		$o = $modx->getObject($obj, $criteria);
		if ($o) {
			$row['store'] = $o->toArray();
			$row['store']['price'] = $miniShop2->formatPrice($row['store']['price']);
		}
